...
if (!$club) {
    setFlashMessage('error', 'Klub tidak ditemukan');
    redirect('index.php');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_club'])) {
    ...
    $chairman = sanitize($_POST['chairman'] ?? '');
    $board_members = sanitize($_POST['board_members'] ?? '');
    $color_primary = sanitize($_POST['color_primary'] ?? '');
    $color_secondary = sanitize($_POST['color_secondary'] ?? '');
    
    // Handle stadium image upload
    $stadium_image = $club['stadium_image'] ?? '';
    if (isset($_FILES['stadium_image']) && $_FILES['stadium_image']['error'] === UPLOAD_ERR_OK) {
        $allowed_types = ['image/jpeg', 'image/png', 'image/jpg', 'image/webp'];
        $file_type = $_FILES['stadium_image']['type'];
        
        if (!in_array($file_type, $allowed_types)) {
            $errors[] = 'Format gambar stadion harus JPG, PNG, atau WEBP';
        } elseif ($_FILES['stadium_image']['size'] > 10 * 1024 * 1024) {
            $errors[] = 'Ukuran gambar stadion maksimal 10MB';
        } else {
            ...
            $file_ext = strtolower(pathinfo($_FILES['stadium_image']['name'], PATHINFO_EXTENSION));
            $new_filename = 'stadium_' . $club['code'] . '_' . time() . '.' . $file_ext;
            $upload_path = $upload_dir . $new_filename;
            
            if (move_uploaded_file($_FILES['stadium_image']['tmp_name'], $upload_path)) {
                // Delete old image if exists
                if (!empty($club['stadium_image']) && file_exists('../../' . $club['stadium_image'])) {
                    @unlink('../../' . $club['stadium_image']);
                }
                $stadium_image = 'uploads/stadiums/' . $new_filename;
            } else {
                $errors[] = 'Gagal mengupload gambar stadion';
            }
        }
    }
    ...
    if (empty($errors)) {
        ...
        $update_stmt->bind_param("ssssisssssssssi", 
            ...
            $stadium_image, $founded_year, $history, $achievements, $owner, $chairman, $board_members,
    ...
    if (isset($_FILES['trophy_photo']) && $_FILES['trophy_photo']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($file_extension, $allowed_extensions)) {
            ...
            if (move_uploaded_file($_FILES['trophy_photo']['tmp_name'], $upload_path)) {
                $trophy_image = 'uploads/trophies/' . $new_filename;
            }
        }
    } else {
        // Fallback to URL if provided
        $trophy_image = sanitize($_POST['trophy_image_url'] ?? '');
    }
    $trophy_years = sanitize($_POST['trophy_years'] ?? '');
...
<div id="tab-identitas" class="tab-content">
    <div class="bg-white rounded-xl shadow-lg p-8">
        ...
        <div class="grid md:grid-cols-2 gap-6">
            ...
            <!-- Stadium Image -->
            <div>
                <label class="block text-sm font-bold text-gray-700 mb-2">🏟️ Gambar Stadion</label>
                <?php if (!empty($club['stadium_image']) && file_exists('../../' . $club['stadium_image'])): ?>
                    <div class="mb-3">
                        <img src="../../<?php echo $club['stadium_image']; ?>" alt="Stadium" class="w-full h-48 object-cover rounded-lg border-2 border-gray-300">
                        <p class="text-xs text-gray-500 mt-1">Gambar stadion saat ini</p>
                    </div>
                <?php endif; ?>
                <input type="file" name="stadium_image" accept="image/*" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-city-blue">
                <p class="text-xs text-gray-500 mt-1">Format: JPG, PNG, WEBP. Maksimal 10MB. Upload gambar baru untuk mengganti.</p>
            </div>
            ...
            <div class="grid md:grid-cols-3 lg:grid-cols-4 gap-6">
                <?php while ($trophy = $trophies_result->fetch_assoc()): ?>
                    ...
                    <div class="trophy-card bg-gradient-to-br <?php echo $gradient_colors; ?> text-white rounded-xl shadow-lg p-6 text-center relative">
                        ...
                        <!-- Delete Button -->
                        <a href="?id=<?php echo $club_id; ?>&delete_trophy=<?php echo $trophy['id']; ?>" onclick="return confirm('Yakin ingin menghapus piala ini?')" class="absolute top-2 right-2 w-8 h-8 bg-red-500 hover:bg-red-600 rounded-full flex items-center justify-center text-white text-sm transition">
                            ✕
                        </a>
