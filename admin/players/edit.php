...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_player'])) {
    ...
    if (empty($errors)) {
        $update_stmt = $db->prepare("UPDATE players SET name = ?, club_id = ?, position = ?, jersey_number = ?, nationality = ?, birth_date = ?, height = ?, weight = ?, biography = ?, joined_date = ?, is_active = ?, photo_url = ? WHERE id = ?");
        
        $update_stmt->bind_param("sssississsisi", $name, $club_id, $position, $jersey_number, $nationality, $birth_date, $height, $weight, $biography, $joined_date, $is_active, $photo_url, $player_id);
