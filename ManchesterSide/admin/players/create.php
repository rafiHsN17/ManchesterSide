...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($_FILES['photo']['type'], $allowed_types)) {
            if ($_FILES['photo']['size'] <= $max_size) {
                ...
                $filename = 'player_' . uniqid() . '_' . time() . '.' . $extension;
                ...
                $errors[] = 'Ukuran foto terlalu besar (max 5MB)';
            }
        } else {
    ...
    if (empty($errors)) {
        $stmt = $db->prepare("INSERT INTO players (name, club_id, position, jersey_number, nationality, birth_date, height, weight, biography, joined_date, is_active, photo_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        
        $stmt->bind_param("sssississsis", $name, $club_id, $position, $jersey_number, $nationality, $birth_date, $height, $weight, $biography, $joined_date, $is_active, $photo_url);
                                ...
                                <div class="grid md:grid-cols-2 gap-4">
                                    ...
                                    <div class="md:col-span-2">
                                        ...
                                        <p class="text-xs text-gray-500 mt-2">
                                            Format: JPG, PNG, WebP, GIF (Max 5MB). Rekomendasi: 400x400px
                                        </p>
                                    </div>
                                </div>
                            </div>
